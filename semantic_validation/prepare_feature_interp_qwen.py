...
MODEL = "qwen"
INPUT_JSON = "result/fineweb_feature_activations_qwen.json"   # your qwen file

# Create output directory if needed
os.makedirs("qwen_output", exist_ok=True)

OUTPUT_JSONL = f"qwen_output/feature_interpret_requests_{OPENAI_MODEL}_{MODEL}.jsonl"
OUTPUT_PREVIEW_CSV = f"qwen_output/feature_snippet_preview_{OPENAI_MODEL}_{MODEL}.csv"
...
def extract_token_window(
    ...
    # Clamp token index if it is out of bounds
    if tok_idx < 0:
        tok_idx = 0
    if tok_idx >= n:
        tok_idx = n - 1
...
def main():
    ...
    print(f"\nWrote batch file: {OUTPUT_JSONL}")
    print(f"Wrote preview CSV: {OUTPUT_PREVIEW_CSV}")
    print(f"Total unique features prepared: {len(seen_pairs)}")
