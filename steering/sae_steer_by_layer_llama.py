...
This script steers a Llama-3.1-8B-instruct model using SAE decoder vectors
...
- For each layer, uses top 10 features within that layer
...
Usage:
    python sae_steer_by_layer_llama.py \
        --csv_path mft_sae_feature_candidates.csv \
        --mfq_path MFQ2.json \
        --output_path results/sae_steering_results.json \
        --n_rollouts 5
...
class SAESteeringManager:
    ...
    Note: Uses llama-3.1-8b-instruct-andyrdt SAEs from SAE_Lens.
    ...
    # SAE repository for Llama-3.1-8B-Instruct (Andy's SAEs)
...
class SAESteeringExperiment:
    ...
    - Uses top 10 features within each layer
    ...
    def run_experiment(
        ...
        For each foundation:
            For each layer:
                Get top 10 features for that layer
        ...
        for foundation in foundations:
            ...
            for layer in self.LAYERS:
                ...
                # Get top 10 features for this foundation and layer
                feature_ids = self.get_top_features_for_layer(df, foundation, layer, top_k=10)
...
def harvest_vectors(
    ...
    mask = (df['Foundation'] == foundation) & (df['Layer'] == layer)
    layer_df = df[mask]
    top_features = layer_df.nlargest(10, 'Cosine_Similarity')
    feature_ids = top_features['Feature_ID'].astype(int).tolist()
