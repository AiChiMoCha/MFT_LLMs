...
def get_cosine_similarities(layer, foundation, device):
    ...
    file_path = (        
        ...
        f"qwen-2.5-7b-instruct_{foundation}_vs_social_norms_enhanced_concept_vector/"
        f"concept_vectors/vectors_npy/"
        f"qwen-2.5-7b-instruct_{foundation}_vs_social_norms_model_layers_{layer-1}_residual_stream_vector.npy"
    )
...
df_sorted.to_csv(f"mft_sae_feature_candidates_{MODEL}.csv", index=False)
df_baseline.to_csv(f"mft_sae_random_baseline_{MODEL}.csv", index=False)

print(f"\nSaved candidates to 'mft_sae_feature_candidates_{MODEL}.csv'")
print(f"Saved baseline to 'mft_sae_random_baseline_{MODEL}.csv'")
