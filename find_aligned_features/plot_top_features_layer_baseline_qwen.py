...
model = 'qwen'
# 2. Load Data
df = pd.read_csv(f"mft_sae_feature_candidates_{model}.csv")
df_baseline = pd.read_csv(f"mft_sae_random_baseline_{model}.csv")
...
for top_num in top_nums:
    ...
    ax.set_xlabel("Layer Index", fontsize=14)
        ...
        # Update legend
    ax.legend(loc='upper left', bbox_to_anchor=(0.1, 1.0), fontsize=14, ncol=2)
    ...
    plt.tight_layout()
    plt.savefig(
        f"avg_cos_sim_top_{top_num}_with_baseline_{model}.pdf",
        dpi=300,
        bbox_inches='tight'
    )
    plt.show()
